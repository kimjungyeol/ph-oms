...
//                judgment.getClassificationId(), judgment.getPsrId());
        ...
        if (userChoisePsrMap == null) {
            judgment.addError("judgment.wo.check", "[[ 품목분류 선택기준 미설정 ]]" + "<br>품목분류 > 선택기준을 설정해야 합니다." + "<br>"
                    + "<br> ** 관련 메뉴 " + "<br>- 관리정보 > 품목분류 > 선택기준");
            return false;
        }
        // 품목분류 > 선택기준 불충족 설정
        if (userChoisePsrMap.get("originAt") == null) {
            judgment.addError("judgment.wo.check", "[[ 품목분류 선택기준 미설정 ]]" + "<br>품목분류 선택기준 미설정으로 불충족 처리되었습니다." + "<br>"
                    + "<br> ** 관련 메뉴 " + "<br>- 관리정보 > 품목분류 > 선택기준");
            return false;
        }
        if (Boolean.valueOf(userChoisePsrMap.get("originAt").toString()) != true) {
            judgment.addError("judgment.wo.check", "[[ 품목분류 선택기준 불충족 ]]" + "<br>품목분류 선택기준 미설정으로 불충족 처리되었습니다." + "<br>"
                    + "<br> ** 관련 메뉴 " + "<br>- 관리정보 > 품목분류 > 선택기준");
            return false;
        }

        judgment.setWoSufficient(true);

        return judgment.getWoSufficient();
    }
